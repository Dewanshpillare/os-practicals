...
abs(initial_head - max_track));
// Traverse the requests in the current direction
...
// If moving to the right end, switch direction and move to the left end
direction = -direction;
total_seek_time += abs(current_head);
current_head = 0;
printf("Move from %d to %d (Seek Time: %d)\n", max_track, 0, max_track);
// Traverse the requests in the opposite direction
for (int i = initial_index + direction; i < n && i >= 0; i += direction) {
int seek = abs(current_head - requests[i]);
total_seek_time += seek;
current_head = requests[i];
printf("Move from %d to %d (Seek Time: %d)\n", current_head + seek,
